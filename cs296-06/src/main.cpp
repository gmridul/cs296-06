...
void gettime(int iter){
    timeval t1,t2; 
    float32 sim_time = 0.0, col_time=0.0, vel_up=0.0, pos_up=0.0;
    float32 time_step=1/settings_hz;
    b2World* sim_world = test->get_world();
    const b2Profile& sim_profile = sim_world->GetProfile();
    gettimeofday(&t1,NULL);
    for(int i=0;i<iter;i++) {
    sim_world->Step(time_step, settings.velocity_iterations, settings.position_iterations);
    sim_time += sim_profile.step;
    col_time += sim_profile.collide;
    vel_up += sim_profile.solveVelocity;
    pos_up += sim_profile.solvePosition;
    }
    gettimeofday(&t2,NULL);
    cout << "Total Iterations: " << iter << endl; 
    cout<<"Average time per step is "<<sim_time/iter<<" ms"<<endl;
		cout<<"Average time for collisions is "<<col_time/iter<<" ms"<<endl;
		cout<<"Average time for velocity updates is "<<vel_up/iter<<" ms"<<endl;
		cout<<"Average time for position updates is "<<pos_up/iter<<" ms\n"<<endl;
		cout << "Total time for loop is "<<(t2.tv_sec - t1.tv_sec)*1000.0 + (t2.tv_usec - t1.tv_usec)/1000.0<< " ms" << endl;
