...
#Finally print all the characters which are not places and do not appear too often along with their frequencies in sorted order
for x in sorted(freq1):
    ...
    if freq1[x] <= threshold:
        continue
    #Print the characters that do not have titles
    if x not in firstNames:
        print(x[0].upper()+x[1:],'-',freq1[x])
    
    #Else find the best title for the character and print it along with its characters
    else:
        title=''
        maxLen=0
        for y in fullNames:
