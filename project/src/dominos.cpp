...
namespace cs296
{
	dominos_t::dominos_t()
	{
		...
		{
			b2PolygonShape p_shape;				
			p_shape.SetAsBox(dist(left,parabola(left,22),left+slide1_i,parabola(left+slide1_i,22))/2.0, 0.05); //! Create a g1_shape as a box g1_l m wide and g1_b m high
			b2BodyDef p_bd;
			p_bd.position.Set(2 +left+slide1_i/2.0, 11 + (parabola(left,22) + parabola(left+slide1_i,18))/2.0); //! Position the body on (g1_x, g1_y)
			p_bd.angle=theta(left,parabola(left,22),left+slide1_i,parabola(left+slide1_i,22))*b2_pi;
			b2Body* p_b2 = m_world->CreateBody(&p_bd); //! Create the body
			p_b2->CreateFixture(&p_shape, 0.0f);
		}
		/*************************************************************/
